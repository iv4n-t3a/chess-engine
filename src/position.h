...
public:
	...
	void do_move(Move);
	void undo_move();
	void generate_pseudolegal_moves(std::vector<Move>& to_add_in);
...
protected:
	void add_moves_by_attack(Square from, Bitboard attack, std::vector<Move>& to_add_in);
	...
	bool is_legal_castling(Move) const;
